---
title: Export & Deploy
description: Share your presentations by exporting to PDF or deploying as static websites.
---

## PDF Export

Export any presentation to a PDF file using the CLI:

```bash
pnpm supaslidev export my-presentation
```

The PDF is saved to `dist/my-presentation.pdf` by default.

### Custom Output Path

Specify a custom output path with the `--output` flag:

```bash
pnpm supaslidev export my-presentation --output ./exports/quarterly-review.pdf
```

The output directory is created automatically if it doesn't exist.

### Export from Dashboard

You can also export presentations through the dashboard UI:

1. Click the **export** button on any presentation card
2. The PDF opens in a new browser tab when complete

::callout{type="info" icon="i-heroicons-information-circle"}
When exporting from the dashboard, PDFs are saved to the `exports/` directory (e.g., `exports/my-presentation.pdf`) rather than `dist/`. This keeps dashboard exports separate from CLI exports.
::

## Deployment

Build all presentations and the dashboard into a unified static site:

```bash
pnpm supaslidev deploy
```

This command:

1. Builds **every presentation** in the workspace using `slidev build`
2. Generates a **PNG thumbnail** of the first slide of each presentation
3. Generates a `presentations.json` catalog from presentation frontmatter (including thumbnail URLs)
4. Builds the Nuxt dashboard in static mode
5. Assembles a single output directory with the dashboard, presentations, and thumbnails

### Output Location

The deployment package is created at `deploy/` by default. Specify a custom location with the `--output` flag:

```bash
pnpm supaslidev deploy --output ./release
```

Set a custom base path for hosting under a subdirectory:

```bash
pnpm supaslidev deploy --base /slides/
```

### Deployment Package Contents

```
deploy/
├── index.html              # Dashboard entry point
├── presentations.json      # Presentation catalog (with thumbnail URLs)
├── presentations/
│   ├── quarterly-review/   # Built presentation (HTML, CSS, JS)
│   ├── product-launch/
│   └── team-onboarding/
├── thumbnails/
│   ├── quarterly-review.webp  # First slide (optimized WebP)
│   ├── product-launch.webp
│   └── team-onboarding.webp
├── vercel.json             # Vercel rewrite rules
├── netlify.toml            # Netlify redirect rules
└── package.json            # Minimal package with serve script
```

### Deploy to Vercel

Deploy the package using the Vercel CLI:

```bash
cd deploy && vercel
```

Or push the deployment folder to Git and import the repository in the Vercel dashboard.

### Deploy to Netlify

Deploy using the Netlify CLI:

```bash
cd deploy && netlify deploy --prod
```

Alternatively, connect your Git repository in the Netlify dashboard and configure the publish directory as `deploy`.

### Preview Locally

Test the built site before deploying:

```bash
cd deploy && npx serve .
```

## Output Locations Summary

| Method                              | Default Output           |
| ----------------------------------- | ------------------------ |
| `pnpm supaslidev export <name>`     | `dist/<name>.pdf`        |
| Dashboard export button             | `exports/<name>.pdf`     |
| `pnpm supaslidev thumbnail <name>`  | `thumbnails/<name>.webp` |
| `pnpm supaslidev deploy`            | `deploy/`                |

::callout{type="tip" icon="i-heroicons-light-bulb"}
Both commands support the `--output` flag to customize the output location.
::

## Deploy Mode

When the dashboard is deployed as a static site (via `pnpm deploy`), it runs in **deploy mode**. In this mode:

- The dashboard reads presentation data from a static `presentations.json` file instead of the API
- Clicking a presentation opens the built static version directly
- Dev server controls (start/stop), export, and edit buttons are disabled and show a "Dev Mode Only" toast when clicked

Deploy mode is activated automatically when the site is built with `pnpm deploy` — no manual configuration is needed.

## Advanced Export Options

Supaslidev uses Slidev's export and build commands under the hood. For advanced configuration options like custom formats, page ranges, or build settings, see the [Slidev Export Documentation](https://sli.dev/guide/exporting).

## Next Steps

::card-group
::card{icon="i-heroicons-photo" to="/features/thumbnails"}
#title
Thumbnails
#description
Generate PNG preview images of your first slides
::

::card{icon="i-heroicons-squares-2x2" to="/features/dashboard"}
#title
Dashboard
#description
Manage and export presentations from the dashboard
::

::card{icon="i-heroicons-arrow-down-tray" to="/features/import"}
#title
Import
#description
Import existing Slidev presentations
::
::
